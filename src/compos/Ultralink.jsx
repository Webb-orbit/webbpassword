...
import { showtost } from "../store/Storeslice.js"
  ...
  const uploadpdf= async(e)=>{
    ...
    try {
      ...
      if (uplo) {
        let baseupload =  await Dataserv.createultratag({'ultraname':uplo.name, 'ulteruserid':selet.userdata, "ultratagfileid":uplo.$id})
        if (baseupload) {
  ...
  const listpdfs = async()=>{
    setload(true)
    if (selet.status) {
      let urls = await Dataserv.allultratags(selet.userdata)
      if (urls) {
        setload(false)
  ...
  const removedotpdf = (file)=>{
    ...
    let splicename =  onlyname.slice(0, 40)
     return splicename
  }
